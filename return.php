		...
		<div id="myTabContent" class="tab-content">
		  	<div class="tab-pane fade active in" id="home">
		  		...
		  		<form class="form-horizontal" action="return.php" method="post">
					...
					<button type="submit" class="btn btn-info" style="width:15%; height:20%; position:fixed; left:15%; top:45%;"><h1><strong>还书</strong></h1></button>
				</form>
		  	</div>
  		</div>
	</div>
...
<?php header("content-type:text/html; charset=utf-8");
	...
	if (($bid!=NULL) || ($cid!=NULL)){
		...
		if (mysql_fetch_row($result)){
			$sql = "update record set rtime='$rtime' where bid='$bid' and cid='$cid'";
			mysql_db_query("test", $sql, $conn);
			echo "<button type='button' class='btn btn-success' style='width:15%; height:20%; position:fixed; left:35%; top:45%;'><h1><strong>还书成功</strong></h1></button>";
		}
		else
			echo "<button type='button' class='btn btn-danger' style='width:15%; height:20%; position:fixed; left:35%; top:45%;'><h1><strong>还书失败</strong></h1></button>";
		mysql_close($conn);
	}
?>